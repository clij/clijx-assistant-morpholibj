...
public abstract class AbstractMorphoLibJAnalyzeRegions extends AbstractCLIJ2Plugin implements CLIJMacroPlugin, CLIJOpenCLProcessor, OffersDocumentation, IsCategorized, HasClassifiedInputOutput
{
    protected String property = null;

    protected AbstractMorphoLibJAnalyzeRegions(String property) {
        this.property = property;
    }

    @Override
    public boolean executeCL() {
        boolean result = morphoLibJAnalyzeRegions(getCLIJ2(), (ClearCLBuffer) (args[0]), (ClearCLBuffer) (args[1]), property);
        return result;
    }
    ...
    protected static boolean morphoLibJAnalyzeRegions(CLIJ2 clij2, ClearCLBuffer labels, ClearCLBuffer parametric_map, String property) {
        // pull image from GPU
        ImagePlus labels_imp = clij2.pull(labels);

        ResultsTable table = new AnalyzeRegions().process(labels_imp);
    ...
    public String getDescription() {
        return "Applies MorphoLibJs Analyse Regions to a label image an produces a parametric image visualizing " + property + ".\n\n" +
